...
interface EmailVariableProps {
    label: string;
    value: string;
    onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
}

export const EmailVariable: React.FC<EmailVariableProps> = ({ label, value, onChange }) => {
    const isValid = !value || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);

    return (
        <NodeViewWrapper as="span" className="inline-block mx-1 align-middle relative my-2">
            <div className={cn(
                "flex flex-col gap-1 px-5 py-3 rounded-[20px] bg-white/[0.03] dark:bg-black/[0.1] backdrop-blur-md border transition-all hover:bg-white/[0.05] focus-within:bg-white/[0.05] min-w-[240px] animate-in fade-in duration-500",
